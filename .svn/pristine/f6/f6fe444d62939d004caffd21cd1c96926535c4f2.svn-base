...
public class ZZJAndCQBIMController {
...
//        try {
            ...
            projectMapper.updateByPrimaryKeySelective(project.setTransformStatus(3));
            return resultData.setStatus("401").setMsg("转换完成");

           /* ZZJ_Token token = zzjFileService.loginZZJ(null, null);
            if(token.getAccess_token()!=null){
                ...
                for (int i = 0; i <sycModel.size() ; i++) {
                    ...
                    res = zzjFileService.transformFile(fileResponse, token.getAccess_token());
                    if (JSONObject.parseObject(res)==null) {
                        throw new Exception("远程网络错误，请稍后重试");//同步出了问题
...
//                        System.out.println("文件已被同步过服务器"+items);//606代表已经转换了的
                        ...
                        String items = JSONObject.parseObject(res).get("items").toString();
                        throw new Exception(StringUtils.isNotBlank(items)?items:"网络错误，稍后重试");
                    }
                }
                ...
                throw new Exception("登陆过期，请刷新页面");
            }
        } catch (Exception e) {
            resultData.setStatus("401").setMsg(e.getMessage()==null?"网络错误请稍后重试":e.getMessage());
            e.printStackTrace();
        }
        return resultData;*/
    }
...
//        try {
        ...
        if(project.getTransformStatus()==3){
            return resultData.setStatus("400").setMsg("已完成转换");
        }
        return resultData;
           /* if(project.getTaskId()==null){//如果项目文件都是删除了再重新转的  那这个工程都没有taskid   就很尴尬
            ...
            ZZJ_Token token = zzjFileService.loginZZJ(null, null);
            if(token.getAccess_token()!=null) {
                String s = zzjFileService.transformStatus(project_id, project.getTaskId(), token.getAccess_token());
                JSONObject jsonObject = JSONObject.parseObject(s);
                if (jsonObject==null || !"200".equals(JSONObject.parseObject(s).getString("code"))) {
                    throw new Exception("远程网络错误，请稍后重试");
                }
            ...
            e.printStackTrace();
            resultData.setStatus("400").setMsg(e.getMessage()==null?"网络错误请稍后重试":e.getMessage());
        }
        return resultData;*/
    }
    ...
    public ResultData upFileToZZJ(String id)throws Exception{
        ...
        Project project = projectService.getProjectById(model.getProject_id());
...
//        }
        ...
        //更新模型
        model.setStatu(2).setModel_file_id("0");//设置模型同步状态为已同步
      ...
      /*  //先登录
        try {
            ...
            while(token.getAccess_token() != null){
                ZZJItem zzjItem = zzjFileService.getFileStatus(md5HashCode,model.getModel_size().intValue()/(1024*1024)+1, token.getAccess_token());//这里有可能token过期
                if(zzjItem==null){
                ...
                if (status == 0) {//未上传
                    try {
                        ...
                        zzjFileResponse = zzjFileService.upFileSync(model, token.getAccess_token());
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    ...
                    //更新模型
                    modelMapper.updateByPrimaryKeySelective(model);
                    status = zzjFileService.sycFile(md5HashCode,token.getAccess_token());
                }
            }

            resultData.setStatus("400").setMsg("登录过期，请重新刷新页面");
        }catch (Exception e) {//2019-12-03 修改
            resultData.setStatus("400").setMsg(e.getMessage()==null?"网络错误请稍后重试":e.getMessage());
            e.printStackTrace();
        }
        return resultData;*/
    ...
    public ResultData upFileToJW(String project_id)throws Exception{
        ...
        /*try {//没有设置 场地部文件上传
            if(project.getTransformStatus()!=3){//但是同步和转换没有关联啊 ，没同步也是可以转化的啊
                return resultData.setMsg("项目未完成，请先转换项目").setStatus("400");
            }
            ...
            for (int i = 0;i<models.size();i++){
                ...
                if(StringUtils.isBlank(s)||!"201".equals(JSONObject.parseObject(s).getString("code"))){
                    ...
                    if(StringUtils.contains(rs,"PRIMARY")){//对已有项目的一次防范，再次点击如果已经同步直接修改状态
                        ...
                        msg.append(nowModel.getModel_name()).append("同步错误:");//.append(rs).append("。\n");
                    }
                }else {
                    modelMapper.updateByPrimaryKeySelective(nowModel.setJWStatus(1));
                }
            ...
            if(StringUtils.isNotBlank(msg)){
                resultData.setStatus("400").setMsg(msg.toString());
            }
        } catch (Exception e) {
            e.printStackTrace();
            resultData.setStatus("400").setMsg(e.getMessage()==null?"网络错误请稍后重试":e.getMessage());
